...
class LLMService:
    ...
    def _identity_context(self, actor: ActorContext) -> str:
        ...
        if channel == "portal":
            lines.append("- ВАЖНО: данные из профиля портала (имя, дети) имеют приоритет над CRM. Если в CRM и профиле портала разные данные о детях — используй данные из профиля.")
        return "\n".join(lines)
