...
## Инструменты агента (function calling) — 11 tools

| Инструмент | Статус | Описание |
|---|---|---|
| `save_user_name` | Готов | Сохранить имя клиента при первом упоминании |
| `search_knowledge_base` | Готов | RAG-поиск по продуктам, ценам, условиям EdPalm |
| `check_client_history` | Готов | Проверка CRM + DMS → тип клиента (new/renewal/reanimation) |
...
| `get_client_profile` | Готов | Профиль клиента из DMS (ФИО, тариф, класс, школа) |
| `generate_payment_link` | Готов | Платёжная ссылка через DMS → Tochka Bank |
| `create_manager_task` | Готов | Задача менеджеру после квалификации + уведомление |
| `register_decline` | Готов | Зафиксировать отказ с причинами из чек-листа |
